...
from transformers.models.llama.modeling_llama import (LlamaAttention, LlamaFlashAttention2,
                                                      LlamaSdpaAttention, LlamaMLP, LlamaRMSNorm)
from transformers.modeling_outputs import BaseModelOutputWithPast, CausalLMOutputWithPast
from transformers.generation.utils import GenerateOutput
from transformers.cache_utils import Cache, DynamicCache

from llava.model_voco.llava_arch import LlavaMetaModel, LlavaMetaForCausalLM
...
class VoCoLlamaModel(LlamaPreTrainedModel):
    """
...
class LlavaVoCoLlamaModel(LlavaMetaModel, VoCoLlamaModel):
    config_class = VocoConfig

    def __init__(self, config: LlamaConfig):
        super(LlavaVoCoLlamaModel, self).__init__(config)


# LlavaMetaForCausalLM is a method class
class VoCoLlamaForCausalLM(LlamaPreTrainedModel, LlavaMetaForCausalLM):
    _tied_weights_keys = ["lm_head.weight"]
    config_class = VocoConfig

    def __init__(self, config):
        super().__init__(config)
        self.model = LlavaVoCoLlamaModel(config)
