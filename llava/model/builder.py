...
def load_pretrained_model(model_path, model_base, model_name, init_vision=True, load_8bit=False, load_4bit=False, device_map="auto",
                          attn_implementation="flash_attention_2", customized_config=None, **kwargs):
    kwargs.update({"device_map": device_map, 'low_cpu_mem_usage': True})
    if kwargs.pop('deepspeed') is not None:
    ...
    if "llava" in model_name.lower():
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                model = LlavaMixtralForCausalLM.from_pretrained(model_base,
                ...
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                model = LlavaMistralForCausalLM.from_pretrained(model_base,
                                                                config=lora_cfg_pretrained,
                                                                attn_implementation=attn_implementation, **kwargs)
                ...
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                model = LlavaGemmaForCausalLM.from_pretrained(model_base,
                                                              config=lora_cfg_pretrained,
                                                              attn_implementation=attn_implementation, **kwargs)
            else:
            ...
            if "mixtral" in model_name.lower():
                ...
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                cfg_pretrained = AutoConfig.from_pretrained(model_path)
                model = LlavaGemmaForCausalLM.from_pretrained(model_base, config=cfg_pretrained,
                                                              attn_implementation=attn_implementation, **kwargs)
            elif (
                ...
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                llava_cfg = LlavaConfig.from_pretrained(model_path)
                model = LlavaLlamaForCausalLM.from_pretrained(model_base, config=llava_cfg,
                                                              **kwargs)
            else:
            ...
            if "mixtral" in model_name.lower():
                ...
                model = LlavaMixtralForCausalLM.from_pretrained(model_path,
                                                                attn_implementation=attn_implementation, **kwargs)
            elif "mistral" in model_name.lower() or "zephyr" in model_name.lower():
                tokenizer = AutoTokenizer.from_pretrained(model_path)
                model = LlavaMistralForCausalLM.from_pretrained(model_path,
                                                                attn_implementation=attn_implementation, **kwargs)
            elif (
                    "wizardlm-2" in model_name.lower()
                    and "vicuna" in model_name.lower()
                ...
                model = LlavaLlamaForCausalLM.from_pretrained(model_path,
                                                              attn_implementation=attn_implementation, config=llava_cfg,
                                                              **kwargs)
                ...
                tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False)
                cfg_pretrained = AutoConfig.from_pretrained(model_path)
                model = LlavaGemmaForCausalLM.from_pretrained(model_path, config=cfg_pretrained,
                                                              attn_implementation=attn_implementation, **kwargs)
            else:
                rank0_print(
        ...
        if model_base is not None:
            ...
            tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
            model = AutoModelForCausalLM.from_pretrained(model_base, torch_dtype=torch.float16, **kwargs)
    ...
    if "llava" in model_name.lower() or init_vision:
        vision_tower = model.get_vision_tower()
        if not vision_tower.is_loaded:
            vision_tower.load_model(device_map=kwargs.get(device_map))
        if device_map != "auto" and device_map is not None:
